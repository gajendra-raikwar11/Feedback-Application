...
router.post("/submitFormResponse/:id", studentValidate, async (req, res) => {
  try {
    const formId = req.params.id;
    const studentId = req.student._id;
    const { answers, formTitle, formType, facultyID, commonSemester } =
      req.body;
    ...
    if (form.deadline && new Date(form.deadline) < currentDate) {
      return res
        .status(400)
        .json({ message: "The deadline for this form has passed" });
    }
    ...
    const { sectionAverages, overallAverage } =
      calculateAverages(processedAnswers);
    ...
    return res
      ...
      .json({
        message:
          "An error occurred while submitting your response: " + error.message,
      });
  }
});
