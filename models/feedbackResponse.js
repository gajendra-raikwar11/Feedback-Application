...
// Helper method to convert text responses to numeric values - Updated to match client-side logic
feedbackResponseSchema.methods.convertResponseToNumeric = function(responseText, questionType) {
  ...
  switch (questionType) {
    ...
    case "grid":
      // Handle grid response format (updated to match client-side logic)
      try {
        // Check if responseText is a string that might be JSON
        if (typeof responseText === 'string') {
          // If it's a simple string representation of a dropdown/radio option
          for (const map of [ratingMap, yesNoMap, weightMap, satisfactionMap, agreementMap, frequencyMap]) {
            if (map[responseText] !== undefined) {
              return map[responseText];
            }
          }
          
          // If it's JSON, try parsing it
          if (responseText.startsWith('{') || responseText.startsWith('[')) {
            ...
            if (typeof parsedValue === 'string') {
              for (const map of [ratingMap, yesNoMap, weightMap, satisfactionMap, agreementMap, frequencyMap]) {
                if (map[parsedValue] !== undefined) {
                  return map[parsedValue];
                }
              }
              return null;
            }
            
            // If it's an object with row-column values (old format)
...
// This function updates the schema with the client-side calculated numeric values
const processFeedbackFormSubmission = (formData) => {
  // Extract and map form data to our schema structure
  const answersWithNumericValues = formData.answers.map(answer => {
    // Check if client has already provided a numeric value
    if (answer.numericValue !== undefined) {
      // Use the value calculated on the client side
      answer.responseNumericValue = answer.numericValue;
    } else {
      // If no numeric value provided, calculate it server-side
      const feedbackInstance = new FeedbackResponse();
      answer.responseNumericValue = feedbackInstance.convertResponseToNumeric(
        answer.responseText || answer.responseOptions?.[0], 
        answer.questionType
      );
    }
  ...
  // Replace answers with the updated array
  formData.answers = answersWithNumericValues;
...
// Create model
const FeedbackResponse = mongoose.model("FeedbackResponse", feedbackResponseSchema);
module.exports = { FeedbackResponse, validateFeedbackResponse, processFeedbackFormSubmission };